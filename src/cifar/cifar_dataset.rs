...
impl CIFAR10Loader for ImageFolderDataset {
    ...
    fn cifar10_train() -> Self {
        let root = "datasets/CIFAR-10-images-master"
            .parse::<PathBuf>()
            .unwrap(); //download();

        /*let items: Vec<(PathBuf, String)> = std::fs::read_dir(root.join("train"))
            .unwrap()
            .map(|x| {
                let x = x.unwrap();
                let name = x.file_name().into_string().unwrap();
                std::fs::read_dir(x.path())
                    .unwrap()
                    .map(|x| (x.unwrap().path(), name.clone()))
                    .sample(&mut rand::rng(), 100) // 5_000
            })
            .flatten()
            .map(|x| {
                [
        ...
        Self::new_classification_with_items(
            items,
            &[
                "airplane",
                "automobile",
    ...
    fn cifar10_test() -> Self {
        return Self::new_classification_with_items::<PathBuf, &str>(vec![], &[]).unwrap();
        let root = "datasets/CIFAR-10-images-master"
            .parse::<PathBuf>()
            .unwrap(); //download();
