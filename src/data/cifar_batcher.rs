...
impl<B: Backend> Batcher<B, ImageDatasetItem, ClassificationBatch<B>> for ClassificationBatcher<B> {
    fn batch(&self, items: Vec<ImageDatasetItem>, device: &B::Device) -> ClassificationBatch<B> {
        ...
        let images1: Tensor<B, 4> = images.clone().sign(); // maps 0 to 0, but idc
        let images2: Tensor<B, 4> = (images - images1.clone() * 0.5).sign(); // maps 0 to 0, but idc

        let images = Tensor::cat(vec![images1, images2], 1);
