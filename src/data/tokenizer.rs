...
impl Default for FalconTokenizer {
    fn default() -> Self {
        //let mut tokenizer = tokenizers::Tokenizer::from_pretrained("gpt2", None).unwrap();
        /*let post_processor = TemplateProcessing::builder()
        .single(Template::try_from("<|begin_of_text|> $0 <|end_of_text|>").unwrap())
        .special_tokens(vec![("<|begin_of_text|>", 17), ("<|end_of_text|>", 11)])
        .build()
        .unwrap();*/

        let mut tokenizer =
            tokenizers::Tokenizer::from_bytes(include_bytes!("../../tokenizer.json")).unwrap();
...
impl Tokenizer for FalconTokenizer {
    ...
    fn start_token(&self) -> usize {
        self.tokenizer.token_to_id("<|begin_of_text|>").unwrap() as usize
    }

    fn end_token(&self) -> usize {
        self.tokenizer.token_to_id("<|end_of_text|>").unwrap() as usize
    }
}
...
mod tests {
    ...
    fn test_encode_decode() {
        let tokenizer = FalconTokenizer::default();
        let text = "A sentence";
